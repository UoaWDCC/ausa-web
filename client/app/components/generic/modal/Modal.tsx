...
const Modal: React.FC<ModalProps> = ({
  ...
  return ReactDOM.createPortal(
    ...
    >
      <div
        className={`relative bg-[#043873] max-w-full flex flex-col rounded-2xl my-auto ${className}`}
      >
        <button
          className="absolute top-8 right-7 sm:top-10 sm:right-10 rounded-full hover:cursor-pointer"
          onClick={onClose}
        >
          <XMarkIcon className="w-5 h-5 text-beige-100 hover:text-steel-blue" />
        </button>
        {children}
      </div>
    </div>,
